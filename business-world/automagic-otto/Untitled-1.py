...
from PIL import ImageFont, Image, ImageDraw  # Added for image handling
...
class VideoProduction:
    def __init__(self, debug_mode=False):
        ...
        self.openai_image_model = os.getenv("OPENAI_IMAGE_MODEL", "dall-e-3") # dall-e-2 is also an option
        self.elevenlabs_model_id = os.getenv("ELEVENLABS_MODEL_ID", "eleven_multilingual_v2")
        self.youtube_category_id = os.getenv("YOUTUBE_VIDEO_CATEGORY_ID", "22") # Default: People & Blogs
        self.youtube_made_for_kids = os.getenv("YOUTUBE_MADE_FOR_KIDS", "False").lower() == 'true'
    ...
    def _load_pil_font(self, size=40, default_font_name='arial.ttf'):
        """Helper function to load a PIL Font, falling back to default."""
        font_path = None
        # Simplified font search logic (expand as needed for cross-platform)
        font_dirs = []
        if os.name == 'nt': # Windows
            font_dirs.append(os.path.join(os.environ.get('WINDIR', 'C:\\Windows'), 'Fonts'))
        else: # Linux, macOS
            font_dirs.extend(['/usr/share/fonts/truetype/dejavu', '/usr/share/fonts/truetype/liberation', '/System/Library/Fonts', '/Library/Fonts'])

        font_names_to_try = [default_font_name, 'DejaVuSans.ttf', 'LiberationSans-Regular.ttf', 'Helvetica.ttf', 'Arial.ttf']
        ...
        try:
            ...
            self.logger.warning(f"Failed to load font {font_path or default_font_name} with size {size}, using default: {e}")
    ...
    def _check_api_keys(self):
        ...
        for env_var, name in required_keys:
            value = os.getenv(env_var) # Ensure value is a string for startswith
            if not value or (isinstance(value, str) and value.startswith("YOUR_")) or value == "":
                missing_keys.append(name)
        
        if missing_keys:
            self.logger.warning(f"Missing or placeholder API keys: {', '.join(missing_keys)}")
            if not self.debug_mode:
                self.logger.error("API keys are required for production mode. Please check API_SETUP_GUIDE.txt for instructions.")
                sys.exit(1)
            else:
                self.logger.info("DEBUG MODE: Proceeding with missing/placeholder API keys. Fallbacks will be used where available.")
        else:
    ...
    def generate_images(self, script):
        ...
        if self.debug_mode and (not os.getenv("OPENAI_API_KEY") or os.getenv("OPENAI_API_KEY").startswith("YOUR_")):
            ...
            for idx in range(1, 4):  # Create 3 test images
                ...
                font = self._load_pil_font(size=60)
        ...
        for idx, prompt in enumerate(image_topics, start=1):
            ...
            try:
                ...
                try:
                    from PIL import Image, ImageDraw, ImageFont
                    # img_path is already defined for the successful case
                    img = Image.new('RGB', (1280, 720), color=(40, 40, 40))
                    d = ImageDraw.Draw(img)
                    font = self._load_pil_font(size=30)
                    d.text((640, 360), f"Image generation failed: {str(e)[:50]}", fill=(255, 0, 0), anchor="mm", font=font)
                    img.save(img_path) # Overwrite with error image
                    image_files.append(img_path) 
                    self.logger.debug(f"Created error placeholder image: {img_path}")
    ...
    def _create_silent_audio_fallback(self, output_path, duration=10):
        """Creates a silent audio file using FFmpeg as a fallback."""
        self.logger.debug(f"Creating silent audio at {output_path} for {duration}s")
        try:
            os.makedirs(os.path.dirname(output_path), exist_ok=True)
            if os.path.exists(output_path): # Remove if already exists to avoid issues
            ...
            subprocess.run(silent_audio_cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE, check=True)
            
            if os.path.exists(output_path) and os.path.getsize(output_path) > 0:
                self.logger.debug(f"Created silent audio: {output_path}")
                return True
            self.logger.error(f"Silent audio created but is empty or invalid: {output_path}")
            return False
        except subprocess.CalledProcessError as e:
            self.logger.error(f"FFmpeg command failed for silent audio '{output_path}': {e.stderr.decode() if e.stderr else e}")
            return False
        except Exception as e:
            self.logger.error(f"Failed to create silent audio {output_path}: {e}")
            return False
    ...
    def generate_voice(self, script):
        ...
        if not self.elevenlabs_client:
            self.logger.error("ElevenLabs client not initialized. Cannot generate voice.")
            # Fallback to silent audio directly
        else:
            # Debug mode with mock audio if API key is placeholder
            if self.debug_mode and (not self.elevenlabs_api_key or self.elevenlabs_api_key.startswith("YOUR_")):
                self.logger.debug("DEBUG MODE: Using placeholder audio as ElevenLabs API key is not fully configured.")
                if self._create_silent_audio_fallback(audio_path, duration=10):
                    self.logger.debug(f"Created silent debug audio: {audio_path}")
                else: # Last resort dummy file
                    with open(audio_path, 'wb') as f: f.write(b'\x00' * 1024)
                return audio_path

            self.logger.info("Generating voice narration via ElevenLabs...")
            try:
                voice_id_env = os.getenv("ELEVENLABS_VOICE_ID")
                voice_to_use = voice_id_env
                if not voice_to_use:
                    voices_response = self.elevenlabs_client.voices.get_all()
                    if voices_response.voices:
                        first_voice = voices_response.voices[0]
                        voice_to_use = first_voice.voice_id
                        self.logger.warning(f"No ELEVENLABS_VOICE_ID in .env, using first available voice: {first_voice.name} ({voice_to_use})")
                    else:
                        raise ValueError("No voices available on your ElevenLabs account")
                
                audio_data = self.elevenlabs_client.generate(
                    text=narration_text, voice=voice_to_use, model=self.elevenlabs_model_id
                )
                with open(audio_path, 'wb') as f: f.write(b"".join(audio_data)) # Correct way to save generator output
                ...
                if self.debug_mode:
                    self.logger.error(traceback.format_exc())
                # Proceed to fallback silent audio generation
        
        # Fallback silent audio generation (if API client failed, was not initialized, or API call failed)
        self.logger.warning("Attempting to create fallback silent audio as primary voice generation failed.")
        if not self._create_silent_audio_fallback(audio_path, duration=20):
            with open(audio_path, 'wb') as f: f.write(b'\x00' * 1024) # Last resort dummy file
        return audio_path
    ...
    def create_video_from_images_and_audio(self, image_files, audio_file):
        ...
        try:
            ...
            with tempfile.NamedTemporaryFile(mode='w', delete=False, suffix='.txt', encoding='utf-8') as tmpf:
                for img_path in image_files:
                    clean_img_path = Path(os.path.abspath(img_path)).as_posix()
    ...
    def upload_to_youtube(self, video_path, title, description):
        ...
        upload_log = script_dir / 'uploaded_videos.log'
        ...
        raw_creds_filename = os.getenv("GOOGLE_API_CREDENTIALS_FILE", "youtube_credentials.json")
        if not raw_creds_filename:
            self.logger.error("GOOGLE_API_CREDENTIALS_FILE is not set in .env. Cannot proceed with YouTube upload.")
            return False
        
        # Ensure it's just a filename, not a potentially incorrect path from .env
        creds_filename = Path(raw_creds_filename).name 
        if not creds_filename.lower().endswith('.json'):
            self.logger.warning(f"GOOGLE_API_CREDENTIALS_FILE '{creds_filename}' does not end with .json. Appending .json.")
            creds_filename += ".json"
            
        credentials_path = script_dir / creds_filename
        token_pickle_path = script_dir / 'token.pickle'

        self.logger.debug(f"Using YouTube credentials file: '{credentials_path}' and token file: '{token_pickle_path}'")
        ...
        creds = None
       # Load credentials from token_pickle_path if it exists
        if os.path.exists(token_pickle_path):
            with open(token_pickle_path, 'rb') as token_file_handle: # Renamed to avoid conflict with token variable
                try:
                    creds = pickle.load(token_file_handle)
                    self.logger.debug(f"Loaded credentials from {token_pickle_path}")
                except (pickle.UnpicklingError, EOFError, AttributeError, ImportError, IndexError) as e:
                    self.logger.error(f"Error unpickling token from {token_pickle_path}: {e}. File might be corrupted or in an old format. Will re-authenticate.")
                    creds = None
                except Exception as e:
                    self.logger.error(f"Unexpected error loading token.pickle: {e}. Will re-authenticate.")
                    if self.debug_mode:
                        self.logger.error(traceback.format_exc())
                    creds = None
    ...
    # It's generally recommended to manage mutable state like DAY_NUMBER
    # in a separate state file (e.g., JSON) rather than modifying .env directly.
    # For simplicity, the current approach is kept, but consider this for future enhancements.
    # Example: self._save_state({"DAY_NUMBER": self.day_number, "SEASON": self.season})
