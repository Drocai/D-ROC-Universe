#!/usr/bin/env python3
"""
Clean YouTube Authentication for AutoMagic
"""
import os
import sys
import webbrowser
...
if os.path.exists('token.pickle'):
    print("[OK] Existing authentication token found")
    print("  Note: We removed the old token, so you'll need to re-authenticate")
else:
...
print()
print("IMPORTANT: You need to authenticate with the Google account")
print("that OWNS this specific channel!")
print()
print("Currently, videos are uploading to the WRONG channel:")
print("Wrong: D RoC (UC79xI_dcS8uz8laX7O6wWdw)")
print("Right: AutoMagic (UC9JN2eg-ja0TOws09jCKHow)")
print()
...
print()
print("You should:")
print("1. Sign in with the Google account that owns the AutoMagic channel")
...
try:
    ...
    print("[OK] Google API libraries loaded")
    
    # Since we removed the old token, we need to authenticate fresh
    print()
    print("BROWSER AUTHENTICATION REQUIRED")
    print("=" * 40)
    print("A browser window will open shortly...")
    print("Please:")
    print("1. Sign in with your Google account that owns the AutoMagic channel")
    ...
    try:
        ...
        with open('token.pickle', 'wb') as f:
            pickle.dump(creds, f)
        print("[OK] Authentication saved for future use")
    ...
    try:
        ...
        if response.get('items'):
            ...
            print(f"[OK] Connected to YouTube channel: {channel_title}")
            ...
            if channel_id == expected_channel_id:
                ...
                print("AutoMagic will now upload videos to the right channel!")
            else:
                print()
                print("[WARNING] This might be the WRONG channel!")
                print(f"Expected: {expected_channel_id}")
                print(f"Got: {channel_id}")
                print()
                print("If this is wrong, run this script again with the correct account.")
        else:
            print("[WARNING] Connected to Google, but no YouTube channel found")
            print("  Make sure you have a YouTube channel set up")
    
    except Exception as e:
        print(f"[WARNING] Could not verify YouTube access: {e}")
    ...
    print("3. Videos should now appear on the correct channel")
